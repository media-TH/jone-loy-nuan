...
let { data: quiz_responses, error } = await supabase
  .from('quiz_responses')
  .select('*')
...
const channels = supabase.channel('custom-delete-channel')
  .on(
    'postgres_changes',
    { event: 'DELETE', schema: 'public', table: 'quiz_responses' },
    (payload) => {
      console.log('Change received!', payload)
    }
  )
...
const channels = supabase.channel('custom-filter-channel')
  .on(
    'postgres_changes',
    { event: '*', schema: 'public', table: 'quiz_responses', filter: 'some_column=eq.some_value' },
    (payload) => {
      console.log('Change received!', payload)
    }
  )
